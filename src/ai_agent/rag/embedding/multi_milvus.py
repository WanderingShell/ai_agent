...
MODEL_PATH = MODELS_DIR / "bge" / "Visualized_base_en_v1.5.pth"
DATA_DIR = DOCS_DIR
...
class Encoder:
    ...
    def __init__(self, model_name: str, model_path: Path):
        self.model = Visualized_BGE(
            model_name_bge=model_name, model_weight=model_path)
        self.model.eval()

    def encode_query(self, image_path: str, text: str) -> list[float]:
        with torch.no_grad():
            query_emb = self.model.encode(image=image_path, text=text)
        return query_emb.tolist()[0]  # type: ignore
...
def visualize_results(query_image_path: str, retrieved_images: list, img_height: int = 300, img_width: int = 300, row_count: int = 3) -> np.ndarray:
    ...
    query_display_area[img_height * (row_count - 1):,
                       :] = cv2.resize(bordered_query, (img_width, img_height))
    ...
    for i, img_path in enumerate(retrieved_images):
        ...
        resized_retrieved = cv2.resize(
            retrieved_cv, (img_width - 4, img_height - 4))
        ...
        # 添加索引号
        cv2.putText(panoramic_image, str(i), (start_col + 10,
                    start_row + 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
...
print("索引详情:")
print(milvus_client.describe_index(
...
query_vector = encoder.encode_query(
    image_path=query_image_path, text=query_text)
